...
class VatratesAPIClientError(Exception):
...
class ValidationError(VatratesAPIClientError):
...
class VatratesAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"country": {"type": "string", "required": True, "minLength": 2, "maxLength": 2}, "date": {"type": "string", "required": False, "format": "date"}, "postcode": {"type": "string", "required": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the VatratesAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises VatratesAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/vatrates'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[VatratesAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns VATRatesData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises VatratesAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise VatratesAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise VatratesAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise VatratesAPIClientError(f"Request failed: {str(e)}")
