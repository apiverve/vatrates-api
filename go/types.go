// Package vatrates provides a Go client for the VAT Rates API.
//
// For more information, visit: https://apiverve.com/marketplace/vatrates?utm_source=go&utm_medium=readme
package vatrates
...
// Request contains the parameters for the VAT Rates API.
//
// Parameters:
//   - country (required): string - ISO 2-letter country code (e.g., DE, FR, ES) [minLength: 2, maxLength: 2]
//   - date: string - Look up rate effective on this date (YYYY-MM-DD). Defaults to today. [format: date]
//   - postcode: string - Check for regional exceptions (e.g., Canary Islands, Ceuta)
type Request struct {
	Country string `json:"country"` // Required
	Date string `json:"date,omitempty"` // Optional
	Postcode string `json:"postcode,omitempty"` // Optional
}
...
// ResponseData contains the data returned by the VAT Rates API.
type ResponseData struct {
	Country string `json:"country"`
	CountryName string `json:"countryName"`
	Currency string `json:"currency"`
	EffectiveFrom string `json:"effectiveFrom"`
	Rates RatesData `json:"rates"`
	Exception interface{} `json:"exception"`
}

// RatesData represents the rates object.
type RatesData struct {
	Standard int `json:"standard"`
	Reduced int `json:"reduced"`
	Reduced2 interface{} `json:"reduced2"`
	SuperReduced interface{} `json:"superReduced"`
